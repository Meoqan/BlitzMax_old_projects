Type Tstar
	...
	Field x:Float, y:Float, d:Float = Rnd(0.8, 1.4), noise:Float = Rnd(-0.1, 0.1)
	Global downlist:TList = CreateList(), stars:Int = 300
	...
	Function Workdown(vx:Float, vy:Float)
		...
		For Local Star:Tstar = EachIn Tstar.downlist
			Star.move(vx, vy)
			If Star.x < 0 Then Star.x:+screenx
			If Star.x > screenx Then Star.x:-screenx
			If star.y < 0 Then star.y:+screeny
			If Star.y > screeny Then Star.y:-screeny
			Star.draw()
		Next
	End Function
	...
	Function Fillup()
		While stars > 0
			...
			star.x = Rnd(0, screenx)
			star.y = Rnd(0, screeny)
			If star.d > 1.0 Then
